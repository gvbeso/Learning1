...
class Database:
    def __init__(self):
        self.con = sqlite3.connect('df.db')
        self.cursor=self.con.cursor()
        self.tabe()

    def tabe(self):
        self.cursor.execute("create table if not exists task( id integer not null primary key AUTOINCREMENT,note varchar(60) not null)")
        self.con.commit()


    def tableinsert(self,note):
        self.cursor.execute("insert into task (note) values ('%s')" % (note))
        self.con.commit()

    def get_tasks(self):
        dbdata = self.cursor.execute("SELECT id, note FROM task").fetchall()
        return dbdata
